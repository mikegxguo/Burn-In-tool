 ...
 * Time: 下午6:37
 */
public class USBTest extends TestClass {

    private File folder;

    private boolean keepTheDiffer;

    public USBTest(Messenger messenger, Activity activity, boolean isContinuous, boolean keepTheDiffer) {
        super(messenger, activity, isContinuous);
        this.id = BISTApplication.USBTest_ID;
...
//        this.logFile = new File(BISTApplication.LOG_PATH, "BIST_Testlog.txt");
    ...
    public USBTest(Messenger messenger, Activity activity, int cycles, boolean keepTheDiffer) {
...
//        this.logFile = new File(BISTApplication.LOG_PATH, "BIST_Testlog.txt");
    ...
    public boolean classSetup() {
        this.folder = new File(BISTApplication.EXT_USB_PATH, "HEAVY_LOADING_TEST");
        if (!folder.exists()) {
            folder.mkdirs();
        }
